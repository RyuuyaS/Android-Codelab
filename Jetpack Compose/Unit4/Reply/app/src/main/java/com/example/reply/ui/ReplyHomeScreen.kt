...
fun ReplyHomeScreen(
    ...
    /*
    There is another way to navigate to another screen, that is use condition expression.
    Here if the screen is the homepage then it will use the ReplyAppContent Composable
        else the screen is in the detail screen then it will use the ReplyDetailsScreen Composable
     */
    if (navigationType == ReplyNavigationType.PERMANENT_NAVIGATION_DRAWER && replyUiState.isShowingHomepage) {
        ...
        if (replyUiState.isShowingHomepage) {
            ...
            /*
            Since we don't use NavHost so we must add the code to create the back button manually
            because we don't have the backstack.
             */
            ReplyDetailsScreen(
                replyUiState = replyUiState,
